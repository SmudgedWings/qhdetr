...
###########################full precision###########################
        if two_stage:
            ...
            self.enc_output = LinearLSQ(d_model, d_model)
            self.enc_output_norm = nn.LayerNorm(d_model)
            # self.pos_trans = nn.Linear(d_model * 2, d_model * 2)
            self.pos_trans = LinearLSQ(d_model * 2, d_model * 2)
...
####################################################################
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, self_attn_mask=None):
        ...
        # encoder
        memory, memory_full = self.encoder(
            src_flatten,
            spatial_shapes,
            level_start_index,
            valid_ratios,
        ...
        return hs, init_reference_out, inter_references_out, None, None, memory, memory_full
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.linear2 = LinearLSQ_v3(d_ffn, d_model, nbits_w=n_bit)
    ...
    def forward_ffn(self, src):
        src2, src2_full = self.linear2(self.dropout2(self.activation(self.linear1(src))))
        # src2 = self.linear2(self.dropout2(self.activation(self.linear1(src))))
        src = src + self.dropout3(src2)
        src = self.norm2(src)
        src_full = src + self.dropout3(src2_full)
        src_full = self.norm2(src_full)
        return src, src_full
        # return src, None
    ...
    def forward(
        ...
        src, src_full = self.forward_ffn(src)
