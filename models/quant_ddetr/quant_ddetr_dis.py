...
from util import box_ops, KD_Loss
...
from .quant_dtransformer_dis import build_deforamble_transformer
...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        return out, feature_q, feature_f.detach()
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, losses, memory=None, memory_full=None, focal_alpha=0.25):
    ...
    def loss_distrubution(self, memory, memory_full):
        criterion_kd = KD_Loss.DistributionLoss()
        loss = criterion_kd(memory, memory_full)
        losses = {"loss_distribution": loss}
        return losses

    def get_loss(self, loss, outputs, targets, indices, num_boxes, memory, memory_full, **kwargs):
        loss_map = {
            "labels": self.loss_labels,
            "cardinality": self.loss_cardinality,
            "boxes": self.loss_boxes,
            "masks": self.loss_masks,
            "distribution": self.loss_distrubution,
        }
        assert loss in loss_map, f"do you really want to compute {loss} loss?"
        if loss == "distribution":
            # pdb.set_trace()
            return loss_map[loss](memory, memory_full)
        else:
            # pdb.set_trace()
            return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def forward(self, outputs, targets, memory, memory_full):
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(
                self.get_loss(loss, outputs, targets, indices, num_boxes, memory, memory_full, **kwargs)
            )
        # pdb.set_trace()
            ...
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                for loss in self.losses:
                    ...
                    l_dict = self.get_loss(
                        loss, aux_outputs, targets, indices, num_boxes, memory, memory_full, **kwargs
                    )
        ...
        if "enc_outputs" in outputs:
            ...
            for loss in self.losses:
                ...
                if loss == "labels":
                    ...
                    kwargs["log"] = False
                l_dict = self.get_loss(
                    loss, enc_outputs, bin_targets, indices, num_boxes, memory, memory_full, **kwargs
                )
...
def build_quant_dis(args):
    ...
    weight_dict["loss_distributuin"] = args.distribution_loss_coef
    if args.masks:
    ...
    losses = ["labels", "boxes", "cardinality", "distribution"]
    if args.masks:
        losses += ["masks"]
