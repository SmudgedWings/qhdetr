...
def train_hybrid(outputs, targets, k_one2many, criterion, lambda_one2many, memory, memory_full):
    # one-to-one-loss
    # pdb.set_trace()
    loss_dict = criterion(outputs, targets, memory, memory_full)
    ...
    # one-to-many loss
    loss_dict_one2many = criterion(outputs_one2many, multi_targets, memory, memory_full)
...
def train_one_epoch(
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        with torch.cuda.amp.autocast() if use_fp16 else torch.cuda.amp.autocast(
            ...
            if use_fp16:
                optimizer.zero_grad()
            outputs, memory, memory_full = model(samples)

            if k_one2many > 0:
                loss_dict = train_hybrid(
                    outputs, targets, k_one2many, criterion, lambda_one2many, memory, memory_full
                )
            else:
                loss_dict = criterion(outputs, targets, memory, memory_full)
